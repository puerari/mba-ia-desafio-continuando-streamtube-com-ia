...
_Clean-slate verification (2026-09-20):_ `docker compose down -v` followed by `docker compose up -d --build` brings the stack up from nothing — six services, with `db`, `minio` and `redis` reporting healthy; `migration:run` applies all three migrations and creates all six tables; the `streamtube` bucket is created automatically by `StorageService.onModuleInit` with no manual MinIO step; the worker logs `consuming queue "video-processing"`. The only first-run wrinkle, now documented in `nestjs-project/CLAUDE.md`: `video-worker` starts before `node_modules` exists and competes with the initial `npm install` over the shared bind mount, so the worker needs one restart after the first install.
